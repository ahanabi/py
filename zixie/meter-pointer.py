...
def calibrate_gauge(gauge_number, file_type):
    ...
    img = cv2.imread('/home/dingzhihui/Downloads/python-cv-samples-master/examples/analog-gauge-reader/images/gauge-%s.%s' %(gauge_number, file_type))
    print(img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    #diff1LowerBound和diff1UpperBound确定线距中心的距离
    ...
    #diff2LowerBound和diff2UpperBound确定线的另一个点与仪表外部的距离
    diff2UpperBound = 1.0
    for i in range(0, len(lines)):
        for x1, y1, x2, y2 in lines[i]:
            #print('x1',x1)
